@extends('layouts.app')
@section('head')
    <link href="{{ asset('module/gastronomia/css/gastronomia.css?_ver='.config('app.version')) }}" rel="stylesheet">
    <link rel="stylesheet" type="text/css" href="{{ asset("libs/ion_rangeslider/css/ion.rangeSlider.min.css") }}"/>
    <link rel="stylesheet" type="text/css" href="{{ asset("libs/fotorama/fotorama.css") }}"/>
@endsection
@section('content')
    <div class="bravo_detail_gastronomia">
        @include('Gastronomia::frontend.layouts.details.gastronomia-banner')
        <div class="bravo_content">
            <div class="container">
                <div class="row py-5">
                    <div class="col-md-12 col-lg-8">
                        <h3>{{$translation->name}}</h3>
                        @include('Gastronomia::frontend.layouts.details.gastronomia-overview')
                    </div>
                ...
                @if(!empty($types))
                    <div class="g-gastronomia-module py-5 border-top border-bottom">
                        <div class="row">
                            <div class="col-12 col-lg-5">
                                <h3>{{__("")}}</h3>
                            </div>
                            <div class="col-12 col-lg-7">
                                <ul class="gastronomia-module-nav nav nav-pills justify-content-lg-end">
                                    @php $i = 0 @endphp
                                    @foreach($types as $type=>$moduleClass)
                                        ...
                                        ->where($moduleInst::getTableName().'.status','publisho')->with('gastronomia')->take(8)->get();
                                        @endphp
            ...
            @include('Gastronomia::frontend.layouts.details.gastronomia-map')
            <div class="container">
                @include('Gastronomia::frontend.layouts.details.gastronomia-trip-idea')
            </div>
        </div>
    </div>
@endsection
