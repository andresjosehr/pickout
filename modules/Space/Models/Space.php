...

{
    ...
    protected $seo_type = 'restaurante';
    ...
    public function getIsOpenAttribute(){
        ...
        $array_create = ['overflow' => true];
        $days = ['mon', 'tue', 'wed', 'thu', 'fri', 'sat', 'sun'];
        foreach($this->hours_open as $key=>$hours_open) {
                switch ($key) {
                    case 'mon':
                        $day_long = 'monday';
                        break;
                ...
                if ($hours_open['open_am'] && $hours_open['close_am']) {
                    $am = $hours_open['open_am']."-".$hours_open['close_am'];      
                } else {
                    $am = "";
                }
                if ($hours_open['open_pm'] && $hours_open['close_pm']) {
                    $pm = $hours_open['open_pm']."-".$hours_open['close_pm'];      
                } else {
                    $pm = "";
                }
                if ($am && $pm) {
                ...
                $array_create += [$day_long => $time];
            
        }

        $openingHours = OpeningHours::create($array_create, '-03:00');
